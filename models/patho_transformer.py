...
def build_pe_lookup(patch_len):
    ...
    rel_pos_width = patch_len  # 55 for input img 224 x 224, after stride-4 downsampling, max delta is 55
...
class ClusterAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads, attn_drop=0., proj_drop=0.,layer_num=0):
        ...
        self.softmax_temperature = [nn.Parameter(torch.tensor(0.3)),1,1,1]
...
class ClusterTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads,
        ...
        self.attn = ClusterAttention(
            ...
            attn_drop=attn_drop, proj_drop=drop,layer_num=layer_num)
...
class ClusterMerging(nn.Module):
    ...
    def __init__(self, dim, out_dim, norm_layer=nn.LayerNorm, alpha=4.0, ds_rate=0.25, reserve_on=True):
        ...
        self.weight_net = nn.Sequential(
            nn.Linear(3, inner_ch, bias=True),
    ...
    def forward(self, pos, feat, member_idx, cluster_mask, learned_prob, stride, pe_idx, reserve_num):
        ...
        keep_num = int(math.ceil(float(n/8)))
        ...
        if learned_prob is not None:
            lp = learned_prob.view(b, n)
            feat=feat*lp.unsqueeze(2)
            # lp = lp * self.alphas
            # final_prob = final_prob + lp
            final_prob = lp####
            # topk_values, topk_indices = torch.topk(lp , k=10)

        # reserve points on a coarse grid
        # if self.reserve_on:
        #     reserve_mask = ((pos % (stride*4)).sum(-1) == 0).float()  # b x n
        #     final_prob = final_prob + (reserve_mask*(-100))
        #     reserve_num=reserve_mask.nonzero(as_tuple=True)[1].shape[0]
        ...
        if not pre_table.is_cuda:
            pre_table = pre_table.to(pe_idx.device)
        weights_table = self.weight_net(pre_table[...,2:])  # 111 x 111 x ic
...
class BasicLayer(nn.Module):
    ...
    def forward(self, pos, feat, h, w, on_grid, stride):
        ...
        """
        h=w=max(h,w)+1
        build_pe_lookup(patch_len=w.detach().cpu())
        ...
        if self.downsample is not None:
            learned_prob = F.softplus(self.prob_net(feat)) # b x n x 1
...
class AutoFocusFormer(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000, embed_dim=[32, 128, 256, 512],
                 ...
                 alpha=4.0, ds_rate=0.25, reserve_on=True,
                 depths=[2, 2, 6, 2], num_heads=[4, 8, 12, 24],
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim[i_layer]),
                               ...
                               cluster_size=cluster_size[i_layer],
        ...
        self.head = nn.Linear(self.num_features, 4) if 1 > 0 else nn.Identity()
        self.attnpool = Attn_Net(L = embed_dim[-1], D = 256, dropout = 0, n_classes = 1)
        self.channl_r=nn.Linear(384,embed_dim[0])
    ...
    def forward_features(self,input):
        ...
        pos = input[1]
        x=self.channl_r(input[0])
        h,w=torch.max(pos[:, :, 0]),torch.max(pos[:, :, 1])
        x = self.pos_drop(x)
        ...
        A, feature = self.attnpool(x.squeeze(0))  # B C 1
