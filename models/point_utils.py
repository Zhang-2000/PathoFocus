...
def space_filling_cluster(pos, m, h, w, no_reorder=False, sf_type='', use_anchor=True):
    ...
    with torch.no_grad():
        ...
        if use_anchor:
            patch_len = (h*w/k)**0.5 #change
            # patch_len=1
            num_patch_h = int(torch.round(torch.tensor(h / patch_len)))
            num_patch_w = int(torch.round(torch.tensor(w / patch_len)))
