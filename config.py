...
# Written by Ze Liu
# --------------------------------------------------------
# Adapted for patho_focus by zy 2023
...
_C.TRAIN.LR_SCHEDULER.DECAY_RATE = 0.01
...
def update_config(config, args):
    ...
    if args.cluster_size:
        config.MODEL.AFF.CLUSTER_SIZE = [ int(item) for item in args.cluster_size]
    if args.nbhd_size:
        config.MODEL.AFF.NBHD_SIZE = [ int(item) for item in args.nbhd_size]
    if args.num_heads:
        config.MODEL.AFF.NUM_HEADS = [ int(item) for item in args.num_heads]
    if args.embed_dim:
        config.MODEL.AFF.EMBED_DIM = [ int(item) for item in args.embed_dim]
    if args.drop_rate:
        config.DROP_RATE =  args.drop_rate
    if args.fold:
        config.fold =  args.fold
    if args.model_type:
        print("fdsaf ",args.model_type)
        config.MODEL.TYPE = args.model_type

    #zidingyi
    # if args.feature_dim:
    #     config.MODEL.FEATURE_DIM = args.feature_dim
    # if args.softmax_temperature:
    #     config.MODEL.SOFTMAX_TEMPERATURE = args.softmax_temperature
